...
from pyspark.sql.functions import lit, col, concat, create_map, round, date_format
...
if __name__ == "__main__":
    ...
    gob_meteor_estacion_mapping = create_map([lit(x) for x in chain(*gob_meteor_estacion_mapping.items())])
    ...
    gob_meteor_metric_mapping = create_map([lit(x) for x in chain(*gob_meteor_metric_mapping.items())])

    # Calculate the average for each hour
    hour_columns = ["H{:02d}".format(i) for i in range(1, 25)]
    avg_hour_expr = sum(col(hour) for hour in hour_columns) / 24

    gob_meteor_output = gob_meteor_df.withColumn("measure", round(avg_hour_expr, 2)) \
                .withColumn("date", concat(col("ANO"), lit("-"), col("MES"), lit("-"), col("DIA"))) \
                .withColumn("station_id", gob_meteor_estacion_mapping.getItem(col("ESTACION"))) \
                .withColumn("metric_id", gob_meteor_metric_mapping.getItem(col("MAGNITUD"))) \
                .select("metric_id", "station_id", "measure", "date")
    ...
    contaminacion_acustica_estacion_mapping = create_map([lit(x) for x in chain(*contaminacion_acustica_estacion_mapping.items())])
    ...
    contaminacion_acustica_metric_mapping = create_map([lit(x) for x in chain(*contaminacion_acustica_metric_mapping.items())])

    # filter tipo=T
    # unpivot metric columns
    # final dataframe

    contaminacion_acustica_output = contaminacion_acustica_df.filter(col("tipo") == "T") \
                .selectExpr("NMT", "anio", "mes", "dia", "tipo", "stack(6, 'LAEQ', LAEQ, 'LAS01', LAS01, 'LAS10', LAS10, 'LAS50', LAS50, 'LAS90', LAS90, 'LAS99', LAS99) as (metric, measure)") \
                .select("NMT", "anio", "mes", "dia", "metric", "measure")
    
    contaminacion_acustica_output = contaminacion_acustica_output.withColumn("date", concat(col("anio"), lit("-"), col("mes"), lit("-"), col("dia"))) \
                .withColumn("station_id", contaminacion_acustica_estacion_mapping.getItem(col("NMT"))) \
                .withColumn("metric_id", contaminacion_acustica_metric_mapping.getItem(col("metric"))) \
                .select("metric_id", "station_id", "measure", "date")
    ...
    open_meteo_estacion_mapping = create_map([lit(x) for x in chain(*open_meteo_estacion_mapping.items())])
    ...
    open_meteo_metric_mapping = create_map([lit(x) for x in chain(*open_meteo_metric_mapping.items())])

    # Convert time column to timestamp
    open_meteo_df = open_meteo_df.withColumn("time", col("time").cast("timestamp"))

    # Group by date and aggregate columns
    open_meteo_output = open_meteo_df \
                ...
                .select(
                    ...
                    col("avg(soil_temperature_0_to_7cm)").alias("soil_temperature_0_to_7cm"),
    ...
    open_meteo_output = open_meteo_output.selectExpr("district", "date", "stack(6, 'snowfall', snowfall, 'snow_depth', snow_depth, 'cloud_cover', cloud_cover, 'soil_temperature_0_to_7cm', soil_temperature_0_to_7cm, 'soil_moisture_0_to_7cm', soil_moisture_0_to_7cm, 'terrestrial_radiation', terrestrial_radiation) as (metric, measure)") \
                .select("district", "date", "metric", "measure")

    
    open_meteo_output = open_meteo_output.withColumn("measure", round(col("measure"), 2)) \
                .withColumn("station_id", open_meteo_estacion_mapping.getItem(col("district"))) \
                .withColumn("metric_id", open_meteo_metric_mapping.getItem(col("metric"))) \
                .select("metric_id", "station_id", "measure", "date")
    ...
    final_df = air_quality_output.unionByName(gob_meteor_output) \
    ...
    final_df.coalesce(1).write.csv(f"/opt/airflow/rich/final_df_{last_Sunday}", header=True) #bitnami/spark
